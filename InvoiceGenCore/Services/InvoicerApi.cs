using InvoiceGenCore.Helpers;
using InvoiceGenCore.Models;
using InvoiceGenCore.Services.Impl;
using System;
using System.Collections.Generic;
using System.Drawing;

namespace InvoiceGenCore.Services
{
    public class InvoicerApi : IInvoicerApi
    {
        ...
        public byte[] GetPdfBytes(string filename, string password = null)
        {
            if (filename == null || !System.IO.Path.HasExtension(filename))
                filename = System.IO.Path.ChangeExtension(Invoice.Reference, "pdf");
            return new PdfInvoice(Invoice).GetPdfBytes(filename, password);
        }
    }
}